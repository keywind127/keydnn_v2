"""
Device-specific implementations of Tensor less-than-or-equal comparison via control-path dispatch.

This module registers CPU and CUDA implementations of the elementwise
less-than-or-equal comparison operator (``__le__``) for tensors.
...
The public operator entrypoint is `TensorMixinComparison.__le__`, and this
...
@tensor_control_path_manager(TMA, TMA.__le__, Device("cuda:0"))
def tensor_le_gpu(self: ITensor, other: Union["ITensor", Number]) -> "ITensor":
    """
    CUDA control path for elementwise less-than-or-equal comparison (Tensor.__le__).
    ...
    if self.device.is_cuda():
        ...
        if hasattr(other, "device"):
            ...
            if not other_t.device.is_cuda():
                self._raise_device_not_supported("le")
            ...
            out = _cuda_le(self, other_t, device=device_index)
            out.requires_grad = False
            return out

        # Tensor-scalar (no scalar->tensor projection)
        if isinstance(other, _Number):
            out = _cuda_le_scalar(self, float(other), device=device_index)
            out.requires_grad = False
            return out
...
@tensor_control_path_manager(TMA, TMA.__le__, Device("cpu"))
def tensor_le_cpu(self: ITensor, other: Union["ITensor", Number]) -> "ITensor":
    """
    CPU control path for elementwise less-than-or-equal comparison (Tensor.__le__).
