"""
Device-specific implementations of Tensor equality comparison via control-path dispatch.

This module registers CPU and CUDA implementations of the elementwise
equality comparison operator (``__eq__``) for tensors. Implementations are
selected at runtime using the `tensor_control_path_manager`, which dispatches
based on the tensor's device.

The public operator entrypoint is `TensorMixinComparison.__eq__`, and this
...
@tensor_control_path_manager(TMA, TMA.__eq__, Device("cuda:0"))
def tensor_eq_gpu(self: ITensor, other: Union["ITensor", Number]) -> "ITensor":
    """
    CUDA control path for elementwise equality comparison (Tensor.__eq__).
    ...
    if self.device.is_cuda():
        ...
        if hasattr(other, "device"):
            ...
            if not other_t.device.is_cuda():
                self._raise_device_not_supported("eq")
            ...
            out = _cuda_eq(self, other_t, device=device_index)
            out.requires_grad = False
            return out

        # Tensor-scalar (no scalar->tensor projection)
        if isinstance(other, _Number):
            out = _cuda_eq_scalar(self, float(other), device=device_index)
            out.requires_grad = False
            return out
...
@tensor_control_path_manager(TMA, TMA.__eq__, Device("cpu"))
def tensor_eq_cpu(self: ITensor, other: Union["ITensor", Number]) -> "ITensor":
    """
    CPU control path for elementwise equality comparison (Tensor.__eq__).
