"""
Device-specific implementations of Tensor not-equal comparison via control-path dispatch.

This module registers CPU and CUDA implementations of the elementwise
not-equal comparison operator (``__ne__``) for tensors. Implementations are
selected at runtime using the `tensor_control_path_manager`, which dispatches
based on the tensor's device.

The public operator entrypoint is `TensorMixinComparison.__ne__`, and this
...
@tensor_control_path_manager(TMA, TMA.__ne__, Device("cuda:0"))
def tensor_ne_gpu(self: ITensor, other: Union["ITensor", Number]) -> "ITensor":
    """
    CUDA control path for elementwise not-equal comparison (Tensor.__ne__).
    ...
    if self.device.is_cuda():
        ...
        if hasattr(other, "device"):
            ...
            if not other_t.device.is_cuda():
                self._raise_device_not_supported("ne")
            ...
            out = _cuda_ne(self, other_t, device=device_index)
            out.requires_grad = False
            return out

        # Tensor-scalar (no scalar->tensor projection)
        if isinstance(other, _Number):
            out = _cuda_ne_scalar(self, float(other), device=device_index)
            out.requires_grad = False
            return out
...
@tensor_control_path_manager(TMA, TMA.__ne__, Device("cpu"))
def tensor_ne_cpu(self: ITensor, other: Union["ITensor", Number]) -> "ITensor":
    """
    CPU control path for elementwise not-equal comparison (Tensor.__ne__).
