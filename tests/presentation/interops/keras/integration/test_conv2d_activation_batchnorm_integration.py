# tests/presentation/interops/keras/integration/test_conv2d_activation_batchnorm_integration.py
"""
...
- Correctly loads Conv2D weights into KeyDNN layout.
- Correctly loads BatchNormalization running statistics and affine parameters.
- Matches deterministic inference behavior against a NumPy NCHW reference.

Notes
-----
- TensorFlow is treated as an optional dependency; tests are skipped when
  TensorFlow is not installed.
- Forward parity is validated against a pure NumPy NCHW reference implementation
  to avoid TensorFlow backend constraints around channels_first execution on CPU.
- BatchNorm parity is tested in evaluation mode only (uses moving statistics).
"""
...
def _batchnorm2d_eval_ref(
    ...
    *,
    ...
    """
    Reference BatchNorm2D inference (evaluation) for NCHW tensors.
    ...
    gamma : np.ndarray | None
        Scale parameter of shape (C,) or None (no scale).
    beta : np.ndarray | None
        Offset parameter of shape (C,) or None (no offset).
    moving_mean : np.ndarray
        Running mean of shape (C,).
    moving_var : np.ndarray
        Running variance of shape (C,).
    eps : float
    ...
    """
    x = np.asarray(x, dtype=np.float32)
    mm = np.asarray(moving_mean, dtype=np.float32).reshape((1, -1, 1, 1))
    mv = np.asarray(moving_var, dtype=np.float32).reshape((1, -1, 1, 1))

    y = (x - mm) / np.sqrt(mv + np.float32(eps))

    if gamma is not None:
        g = np.asarray(gamma, dtype=np.float32).reshape((1, -1, 1, 1))
        y = y * g
    if beta is not None:
        b = np.asarray(beta, dtype=np.float32).reshape((1, -1, 1, 1))
        y = y + b
...
def _set_batchnorm_eval_if_present(layers: List[Any]) -> None:
    """
    Set `.training = False` for any KeyDNN BatchNorm modules in the layer list.
    ...
    for m in layers:
        ...
        if name in ("BatchNorm1d", "BatchNorm2d") and hasattr(m, "training"):
            m.training = False


@unittest.skipUnless(
    _tf_available(),
    "TensorFlow not installed; skipping Keras Conv2D+BN integration tests.",
)
class TestKerasConv2DActivationBatchNormIntegration(TestCase):
    """
    Integration tests for conversion and forward parity of Conv2D + Activation + BatchNorm.
    ...
    def _build_model(self):
        """
        Build a Keras Sequential model with Conv2D(valid) -> ReLU -> BatchNormalization.
        ...
        # Build entire model if needed.
        if not getattr(model, "built", False):
            model.build((None, 1, 5, 5))

        return model

    def test_conv2d_relu_batchnorm_eval_parity_numpy_ref(self):
        """
        Converted KeyDNN pipeline should match a NumPy reference for inference behavior.

        BatchNorm is forced to evaluation mode in KeyDNN (uses running stats),
        matching `training=False` inference semantics in the NumPy reference.
        """
        tf = self.tf
        ...
        conv_kernel = (
            np.arange(3 * 3 * 1 * 2, dtype=np.float32).reshape((3, 3, 1, 2)) * 0.1
        )
        conv_bias = np.array([0.25, -1.0], dtype=np.float32)
        model.layers[0].set_weights([conv_kernel, conv_bias])

        # Keras BN weights order: gamma, beta, moving_mean, moving_variance
        gamma = np.array([1.5, 0.5], dtype=np.float32)
        beta = np.array([0.1, -0.2], dtype=np.float32)
        moving_mean = np.array([0.25, -0.75], dtype=np.float32)
        moving_var = np.array([2.0, 0.5], dtype=np.float32)
        model.layers[2].set_weights([gamma, beta, moving_mean, moving_var])

        # -------------------------
        # Build NumPy reference
        # -------------------------
        x_np = (
            np.arange(1 * 1 * 5 * 5, dtype=np.float32).reshape((1, 1, 5, 5)) - 12.0
        ) * 0.05

        # Convert Keras conv kernel to KeyDNN layout:
        ...
        y1 = _relu(y0)
        y_ref = _batchnorm2d_eval_ref(
            y1,
            gamma=gamma,
            beta=beta,
            moving_mean=moving_mean,
            moving_var=moving_var,
            eps=float(getattr(model.layers[2], "epsilon", 1e-3)),
        )
        ...
        _set_batchnorm_eval_if_present(layers)
