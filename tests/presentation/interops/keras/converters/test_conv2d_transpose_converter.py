# tests/presentation/interops/keras/converters/test_conv2d_transpose_converter.py
"""
Unit tests for the Keras -> KeyDNN Conv2DTranspose converter.

These tests verify that:
- Keras Conv2DTranspose configuration maps correctly to KeyDNN Conv2dTranspose hyperparameters.
...
- Unsupported Keras configurations are rejected in Phase 1:
  ...
  - padding="same" with output_padding != (0, 0)
...
from src.keydnn.presentation.interops.keras.converters.conv2d_transpose import (
...
class TestKerasConv2DTransposeConverter(TestCase):
    """
    Test suite for `Conv2DTransposeConverter`.
    ...
    def _build_keras_conv2d_transpose_layer(
        self,
        *,
        in_channels: int,
        out_channels: int,
        ...
        """
        Create and BUILD a Keras Conv2DTranspose layer by calling `layer.build(...)`.
        ...
        output_padding : Optional[tuple], optional
            Output padding (op_h, op_w) or None.
        ...
        -------
        Any
            Built Keras Conv2DTranspose layer instance.
        """
        tf = self.tf
        ...
        if groups != 1:
            setattr(layer, "groups", int(groups))
    ...
    def test_build_maps_hyperparams_valid_padding(self):
        """
        Converter.build should map Keras Conv2DTranspose hyperparameters to KeyDNN Conv2dTranspose.
        """
        k_layer = self._build_keras_conv2d_transpose_layer(
            in_channels=3,
            out_channels=5,
            kernel_size=(3, 3),
            strides=(2, 2),
        ...
        self.assertEqual(tuple(getattr(kd, "output_padding")), (1, 0))

        self.assertIsNotNone(getattr(kd, "weight", None))
        self.assertEqual(tuple(kd.weight.shape), (3, 5, 3, 3))
    ...
    def test_build_maps_same_padding_stride1_odd_kernel_output_padding_zero(self):
        """
        padding='same' should map to static padding (k//2, k//2) when stride=1,
        odd kernels, and output_padding is zero.
        """
        k_layer = self._build_keras_conv2d_transpose_layer(
            in_channels=2,
            out_channels=4,
        ...
        self.assertEqual(tuple(getattr(kd, "output_padding")), (0, 0))
    ...
    def test_load_weights_transposes_kernel_and_copies_bias(self):
        ...
        # Keras Conv2DTranspose kernel layout: (k_h, k_w, out_c, in_c)
        kernel = np.arange(k_h * k_w * out_c * in_c, dtype=np.float32).reshape(
            (k_h, k_w, out_c, in_c)
        )
    ...
    def test_build_rejects_same_padding_with_nonzero_output_padding(self):
        """
        padding='same' with output_padding != (0,0) should be rejected in Phase 1.
        """
        k_layer = self._build_keras_conv2d_transpose_layer(
            in_channels=3,
            out_channels=4,
