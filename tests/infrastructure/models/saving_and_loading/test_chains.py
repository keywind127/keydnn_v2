...
def _tensor_from_numpy(arr: np.ndarray, *, device) -> Tensor:
    """
    Create a Tensor using public APIs only.
    """
...
class TestModelSaveLoadChainedModelsJSON(unittest.TestCase):
    def test_save_load_json_rnn_linear_activation_forward_matches(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as td:
            ckpt = Path(td) / "rnn_linear_sigmoid.json"
    ...
    def test_save_load_json_cnn_linear_activation_forward_matches(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as td:
            ckpt = Path(td) / "cnn_linear_softmax.json"
            model.save_json(ckpt)
    ...
    def test_save_load_json_two_branchlike_stacks_sequential_order_preserved(
        self,
    ) -> None:
        """
        A slightly deeper chain to catch ordering issues:
        ...
        model = Sequential(
            Conv2d(in_channels=1, out_channels=2, kernel_size=3, padding=1, bias=False),
            ReLU(),
            Conv2d(in_channels=2, out_channels=2, kernel_size=3, padding=1, bias=True),
            ReLU(),
            MaxPool2d(kernel_size=2, stride=2, padding=0),
            Flatten(),
        ...
        with tempfile.TemporaryDirectory() as td:
            ckpt = Path(td) / "deep_cnn_chain.json"
            model.save_json(ckpt)
            loaded = Sequential.load_json(ckpt)
