...
class TestMemcpyCudaOps(unittest.TestCase):
    ...
    def test_htod_and_dtoh_roundtrip_float32(self) -> None:
        ...
        try:
            self.memcpy_htod(
                self.env.lib, dst_dev=int(dev), src_host=x, nbytes=nbytes, sync=True
            )

            out = np.empty_like(x)
            self.memcpy_dtoh(
                self.env.lib, dst_host=out, src_dev=int(dev), nbytes=nbytes, sync=True
            )
    ...
    def test_dtod_copies_bytes(self) -> None:
        ...
        try:
            ...
            self.memcpy_htod(
                self.env.lib, dst_dev=int(a), src_host=x, nbytes=nbytes, sync=True
            )

            self.memcpy_dtod(
                self.env.lib, dst_dev=int(b), src_dev=int(a), nbytes=nbytes, sync=True
            )

            out = np.empty_like(x)
            self.memcpy_dtoh(
                self.env.lib, dst_host=out, src_dev=int(b), nbytes=nbytes, sync=True
            )
    ...
    def test_invalid_nbytes_raises(self) -> None:
        ...
        try:
            with self.assertRaises((ValueError, TypeError)):
                ...
                self.memcpy_htod(
                    self.env.lib, dst_dev=int(dev), src_host=x, nbytes=-1, sync=True
                )
        finally:
            self._free(dev)
